  ...
  beforePrint() {
    ...
    if (!this.pdfViewer.pageViewsReady) {
      this.l10n.get('printing_not_ready', null,
        'Warning: The PDF is not fully loaded for printing.').then((notReadyMessage) => {
...
if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
  ...
  validateFileURL = function validateFileURL(file) {
    ...
    try {
      ...
      PDFViewerApplication.l10n.get('loading_error', null,
        'An error occurred while loading the PDF.').then((loadingErrorMessage) => {
...
function webViewerInitialized() {
  ...
  function touchStart(event) {
    this.startY = _getEvent(event).touches[0].pageY;
    this.startX = _getEvent(event).touches[0].pageX;
  }

  function touchMove(event) {
    const movementY = abs(_getEvent(event).touches[0].pageY - this.startY);
    const movementX = abs(_getEvent(event).touches[0].pageX - this.startX);
  ...
  function touchEnd(event) {
    const movementY = _getEvent(event).changedTouches[0].pageY - this.startY;
    const movementX = _getEvent(event).changedTouches[0].pageX - this.startX;
    if (movementX < -80 && movementY < 60) {
  ...
  appConfig.mainContainer.addEventListener('touchstart', touchStart);
  appConfig.mainContainer.addEventListener('touchmove', touchMove);
  appConfig.mainContainer.addEventListener('touchend', touchEnd);
